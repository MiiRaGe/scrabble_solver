...
def get_twelveth_words(letters):
    multiplier = [1, 1, 1, 1, 1, 3, 1, 1]
    twelveth_words = [x for x in ALL_WORDS[7] if points(x, multiplier, 2) <= 28]
    ...
    twelveth_words = [x for x in twelveth_words if points(x, multiplier, 2) + 8 + VALUES[x[3]] == 28]
